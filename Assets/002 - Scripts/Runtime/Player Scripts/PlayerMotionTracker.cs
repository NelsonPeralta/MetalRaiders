...
public class PlayerMotionTracker : MonoBehaviourPun
{
    ...
    public void Setup()
    {
        //PrintOnlyInEditor.Log($"PlayerMotionTracker for {player.name}: Setup {_motionTrackerDotsList.Length} {GameManager.instance.GetAllPhotonPlayers().Count}");

        for (int i = 0; i < _motionTrackerDotsList.Length; i++)
        {
            if (i < 8)
            {
                //PrintOnlyInEditor.Log($"PlayerMotionTracker for {player.name}: Setup {_motionTrackerDotsList[i].targetPlayerController}");
