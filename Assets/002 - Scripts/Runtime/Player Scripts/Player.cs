...
public class Player : Biped
{
    ...
    public float hitPoints
    {
        ...
        set
        {
            ...
            //PrintOnlyInEditor.Log($"damage: {newValue} {hitPoints} {_damage} {overshieldPoints} {_overshieldRecharge} {maxOvershieldPoints}");
    ...
    {
        get { if (playerInventory.activeWeapon.killFeedOutput == WeaponProperties.KillFeedOutput.Oddball || playerInventory.activeWeapon.killFeedOutput == WeaponProperties.KillFeedOutput.Sword) return 250; return _meleeDamage; }
    }
    ...
    public float defaultVerticalFov
    {
        ...
        private set
        {
            ...
            {
                if (rid == 0) _defaultVerticalFov = 31.42f; // too fish eye
    ...
    void AchievementCheck(int sourcePid)
    {
        if (!PV.IsMine && sourcePid == GameManager.GetRootPlayer().photonId)
            if (_killFeedOutput == WeaponProperties.KillFeedOutput.Plasma_Blaster
                || _killFeedOutput == WeaponProperties.KillFeedOutput.Plasma_Rifle
                || _killFeedOutput == WeaponProperties.KillFeedOutput.Plasma_Pistol)
            {
    ...
    public void ChangePlayerIdLocalMode(int id) // Only works when no internet connection
    {
        ...
        //{
        //    PrintOnlyInEditor.Log("ChangePlayerIdLocalMode 1");
        //    _playerId = id;
        //    //OnPlayerIdAssigned?.Invoke(this);
        //    PrintOnlyInEditor.Log("ChangePlayerIdLocalMode 2");
        //}
    }
