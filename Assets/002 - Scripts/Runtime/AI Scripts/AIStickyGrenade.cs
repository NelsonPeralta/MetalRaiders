    ...
    private void OnTriggerEnter(Collider collision)
    {
        if (!explosionTimerStarted && !hasHitObject)
        {
            if (collision.gameObject.layer == 13)
            {
                ...
                {
                    if (collision.gameObject.GetComponent<PlayerHitbox>().player.GetComponent<Player>().controllerId != playerRewiredID)
                    {
        ...
        foreach (Collider hit in colliders)
        {
            ...
            if (hit.GetComponent<PlayerHitbox>() != null)
            {
                ...
                for (int i = 0; i < playersHit.Length; i++)
                {
                    ...
                    {
                        if (playerHitID == playersHit[i].GetComponent<Player>().controllerId)
                        {
