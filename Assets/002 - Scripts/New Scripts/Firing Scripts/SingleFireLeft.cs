...
public class SingleFireLeft : MonoBehaviour
{
    ...
    public void Start()
    {
        ...
        if (ThisIsShootingLeft)
        {
            ...
            if (!dwLeftWP.usesGrenades && !dwLeftWP.usesRockets)
            {
    ...
    public void Update()
    {
        if (pController.isDualWielding && pInventory.leftWeapon.GetComponent<WeaponProperties>().isSingleFire)
        {
            nextFireInterval = pInventory.leftWeapon.GetComponent<WeaponProperties>().timeBetweenSingleBullets;

            dwLeftWP = pController.dwLeftWP;

            if (pController.isShootingLeft && !ThisIsShootingLeft && !hasLeftButtonDown)
            {
                Debug.Log("Tracker 1");
                StartCoroutine(SingleFireVoid(false, true));
