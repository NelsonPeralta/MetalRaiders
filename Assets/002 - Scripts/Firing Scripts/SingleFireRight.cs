...
public class SingleFireRight : MonoBehaviour
{
    ...
    public void Update()
    {
        if (pController.isDualWielding && pInventory.rightWeapon.GetComponent<WeaponProperties>().firingMode == WeaponProperties.FiringMode.Single)
        {
            ...
            if (pController.isShootingRight && !ThisIsShootingRight && !hasRightButtonDown)
            {
                StartCoroutine(SingleFireVoid(true, false));
